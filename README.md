...
> 小型個股期貨(如小台達電 `FIRVF`)每口 100 股,微型台指期貨(`FITM`)每點
> 10 元。套錯乘數就是好幾倍的誤差。
>
> **刻意不提供預設乘數**。曾經有 `default_multiplier: 100`,結果新建倉的微台指
> 被默默套成 100,部位價值灌成 10 倍、曝險倍數跟著膨脹成好幾倍;程式雖有
> 印警告,但排程在背景跑沒人看得到。現在遇到沒設定的 symbol,快照會直接中止,
> 錯誤訊息會列出要補的代號,並附上用券商損益反推的乘數
> (`乘數 = 損益 ÷ (市價 − 成本) ÷ 口數`;剛建倉沒價差時無法反推,要查契約規格)。
> 寧可缺一天資料,也不要寫進錯 10 倍的數字。
>
...
> ⚠️ **價格要讀 `fast_info.lastPrice`,不能只讀 `history()`。**
> Yahoo 的歷史收盤檔更新很慢——當日收盤後那一列的 `Close` 可能仍是 NaN
> (實測日股收盤後數小時仍為空),`dropna()` 濾掉後就退回前一個交易日,
> 導致海外持股的價格**每天都慢一個交易日**而不易察覺。
> `fast_info` 的即時報價當下就有值,所以優先讀它,歷史檔只當後備
> (冷門標的或 Yahoo 限流時);退回後備時會印提醒。
>
> FastInfo 的存取方式在不同 yfinance 版本不一致(`fi["lastPrice"]`、
> `fi.get("lastPrice")`、`fi.last_price`),而且可能其中一種回 `None`,
> 所以三種都要試過才放棄。
