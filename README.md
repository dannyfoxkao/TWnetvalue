...
- 富邦:`query_settlement(account, "3d")`,取 `details` 裡
  **`settlement_date` 晚於今天**的 `total_settlement_amount`
  (負數=應付/買進、正數=應收/賣出;沒成交的日期欄位全是 `None`,要略過)
- 永豐:`api.settlements()` 回傳 T+0/1/2 三筆,**只取 T>0**

> ⚠️ **券商在扣款後不會把該筆移出交割清單**。它是「哪天成交、哪天交割」的
> 歷史紀錄,會一直留在三天的查詢視窗內,所以**清單本身分辨不出扣款與否**。
> 同一筆的實測(08/12 買進、08/14 交割):
>
> | 時間 | 交割戶餘額 | 清單上該筆 | 應否計入 |
> |---|---|---|---|
> | 交割日 00:47 | 210,146 | 掛著 | 要(錢還沒扣) |
> | 交割日 22:46 | 9,126 | **仍掛著** | 不要(錢已扣) |
>
> 因此只能靠「交割日是否已過」判斷,而**這個判斷成立的前提是快照在扣款
> 完成後才跑**(台股約在交割日上午完成)。`snapshot.py` 因此內建執行時間
> 檢查:平日 14:00 前執行會中止並提示,要跑得加 `--force`;排程設在 16:32
> 不受影響。
